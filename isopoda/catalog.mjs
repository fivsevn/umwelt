...
import {renderCredits} from './credits.mjs?v=species-39b';
...
export function renderCatalog(p,{unlocked=true,collectedOn=null}={}){
 ...
 card.append(text('p','标本 / '+p.id.toUpperCase(),'specimen-id'),art,text('p',p.game?.referenceOnly?'档案类型  海洋参考标本':'采集日期  '+(collectedOn||'未记录'),'specimen-date'));
 ...
 if(p.game?.referenceOnly)names.append(text('p','MARINE REFERENCE · 海洋物种，不进入陆生饲养盒抽取。','name-aside'));
