...
for(const h of HABITATS.filter(h=>h.aquatic))for(let turn=0;turn<(h.turns??h.days*3);turn++){
 const scene=aquaticScene({habitatId:h.id,day:Math.floor(turn/3)+1,period:turn%3,records:Array.from({length:turn},()=>({kind:'abyssal-dialogue'}))});
...
// Exact fragments are only meaningful after composition. Validate representative complete
// strings through the real translator instead of suppressing arbitrary source warnings.
const engineFragments=new Map([
 ['一个个体','今天一个个体曾经离开土面。盒子里的移动因此停顿了一会儿。'],
 ['这一次与你的猜测相同。','它最终靠近湿苔。这一次与你的猜测相同。地面的差别还在那里。'],
 ['与你留下的箭头不同。','它最终靠近湿苔。与你留下的箭头不同。地面的差别还在那里。']
]);
const engineIgnore=new Set(['晨间','午后','夜间']); // internal PERIODS state labels
const engineStrings=chineseStrings(engineSource);
for(const value of engineStrings){
 if(engineIgnore.has(value))continue;
 const sample=engineFragments.get(value)||value;
 for(const lang of ['en','ja'])if(!translatedZh.has(value)&&gameText(sample,lang)===sample)fail(`engine ${lang}: untranslated source: ${value}`);
}
for(const value of engineFragments.keys())if(!engineStrings.includes(value))fail(`engine: stale fragment allowance: ${value}`);
...
if(errors.length){
 ...
 const aquaticHabitats=HABITATS.filter(h=>h.aquatic),aquaticSpecimens=new Set(aquaticHabitats.flatMap(h=>h.species||[])).size,aquaticScenes=aquaticHabitats.reduce((n,h)=>n+(h.turns??h.days*3),0);console.log(`[i18n] OK — ${baseKeys.length} UI keys, ${speciesIds.size} registry specimens (${aquaticSpecimens} aquatic) / ${aquaticScenes} scenes, ${annotationIds.length} annotation entries, ${rowTriples.length} game locale rows, ${Object.keys(creditFiles).length} credit files.`);
}
