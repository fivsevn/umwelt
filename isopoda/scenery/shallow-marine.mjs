...
export function createSeaweedBed(items){return {items:items.filter(o=>o.layered&&o.kind==='kelp'),bends:new Map(),held:null,frame:null,events:{attached:0,crossed:0,hidden:0,reappeared:0,plucked:0},time:0}}
export function seaweedFrame(bed,{time=bed.time,flow=46,reduced=false}={}){
 bed.time=time;const plants=bed.items.map(item=>kelpGeometry(item,{time,flow,motion:reduced?.18:1,bend:bed.bends.get(item.id)||0}));
 return bed.frame={plants,byId:new Map(plants.map(p=>[p.id,p])),mask:kelpDepth(plants)};
}
...
export function stageSeaweed(group,bed){
 const plants=[...bed.frame.plants].sort((a,b)=>a.item.y-b.item.y||a.item.x-b.item.x);
 group.forEach((a,i)=>{
...
function closestSurface(frame,a,exclude=null,group=[]){
 let best=null;
 for(const plant of frame.plants){if(plant.id===exclude)continue;for(let surface=1;surface<plant.surfaces.length;surface++)for(const t of [.25,.6,.85]){
