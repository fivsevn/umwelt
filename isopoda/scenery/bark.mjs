...
export function drawBark(ctx,{x=192,y=218,a=-.08,scale=1,variant=0,lift=0,seed=57,tone='normal'}={}){
 ...
 const waterlogged=tone==='waterlogged';
 const wood=waterlogged
  ? {edge:'#574636',deep:'#45392c',dark:'#514131',mid:'#5f4c37',grain:'#6b563c',light:'#786246',high:'#826c4d'}
  : {edge:'#68533d',deep:'#534532',dark:'#63503a',mid:'#735b40',grain:'#806747',light:'#8e7350',high:'#9b815b'};
 ...
 paint(ctx,{x,y:y+lift,seed,texture:'wood',a,scale,extent:half+8,inside,edge:wood.edge,shade:(u,v)=>{
  ...
  if(knot<1.1)return knot>.88?wood.light:knot>.65?wood.mid:knot>.35?wood.dark:wood.deep;
  if(v>bottom-2.5)return wood.deep;
  if(v<top+1.5)return wood.high;
  ...
  if(wave<-.76&&Math.sin(u*.16+ridge*.2)>.05)return wood.deep;
  if(wave>.73&&grain%4!==0)return wood.high;
  if(wave>.28)return wood.light;
  if(wave<-.3)return wood.dark;
  return grain%5===0?wood.grain:wood.mid;
 }});
}
