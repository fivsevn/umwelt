...
export function drawTwig(ctx,{x=0,y=0,a=0,length=18,seed=0}={}){
 ...
 paint(ctx,{x,y,a,extent:length+8,inside:(u,v)=>u>=0&&u<length&&Math.abs(v-curve(u))<1.65-u/length*.8||nearLine(u,v,length*.48,curve(length*.48),length*.79,-9,.7),edge:null,shade:(u,v)=>hash32(seed,Math.floor(u/3))%7===0?'#68533e':v<curve(u)-.4?'#8b7350':'#574332'});
}
export function drawWoodChip(ctx,{x=0,y=0,a=0,variant=0,scale=1,seed=0}={}){
 ...
 paint(ctx,{x,y,a,scale,extent:28,inside:(u,v)=>contains(roundedShape,u+Math.sin(v*.43+seed)*.55,v+Math.sin(u*.38+seed)*.65),edge:'#684b36',shade:(u,v)=>Math.sin(v*1.4+Math.sin(u*.21))+Math.sin(u*.39+seed)*.5>.7?'#927953':v<Math.sin(u*.2)?'#806747':'#65523b'});
}
