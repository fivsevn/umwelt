import {makeIndividuals,stageIndividuals,stepIndividuals} from './behaviors.mjs?v=fieldnotes-5';
import {encounterById,responseMode} from './encounters.mjs?v=fieldnotes-5';
import {makeIsopod,setIsopodState} from './sprites.mjs?v=fieldnotes-5';
import {speciesById} from './species.mjs?v=fieldnotes-5';
...
function reset(options={}){
 ...
 encounter=empty?null:encounterById(state.scene?.encounter);stageIndividuals(critters,encounter);drawHabitat(0);
}
function stage(scene){encounter=encounterById(scene.encounter);elapsed=0;effect=null;stageIndividuals(critters,encounter);drawHabitat(0)}
function react(id){effect={id,mode:responseMode(id),until:elapsed+8}}
function present(){
 const rect=canvas.getBoundingClientRect();if(!rect.width||!rect.height)return;
 const w=Math.max(80,Math.round(rect.width/2)),h=Math.max(60,Math.round(rect.height/2));
 if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h}
 display.imageSmoothingEnabled=false;
 const sw=384/camera.zoom,sh=430/camera.zoom;
 ...
 const scaleX=rect.width/sw,scaleY=rect.height/sh,scale=Math.min(scaleX,scaleY);
 for(const c of critters){const x=(c.x-sx)*scaleX,y=(c.y-sy)*scaleY;const offscreen=x< -50||y< -50||x>rect.width+50||y>rect.height+50;c.el.style.visibility=c.hidden||offscreen?'hidden':'visible';setIsopodState(c.el,{posture:c.posture,molt:c.molt,moving:active&&!c.hidden&&!offscreen&&c.moving&&!reduced});c.el.dataset.activity=c.activity||'rest';c.el.classList.toggle('grooming',active&&c.groom&&!c.hidden&&!offscreen&&!reduced);c.el.style.transform='translate('+Math.round(x)+'px,'+Math.round(y+(c.lift||0))+'px) translate(-50%,-50%) rotate('+c.a+'rad) scale('+scale*c.size+')'}
}
function zoom(z){camera.zoom=Math.max(1,Math.min(3,z));present();return camera.zoom}
document.addEventListener('visibilitychange',()=>{if(document.hidden)for(const c of critters){setIsopodState(c.el,{posture:c.posture,molt:c.molt,moving:false});c.el.classList.remove('grooming')}});
let pointer=null;
canvas.addEventListener('pointerdown',e=>{pointer={x:e.clientX,y:e.clientY};canvas.setPointerCapture(e.pointerId)});
canvas.addEventListener('pointermove',e=>{if(!pointer)return;const r=canvas.getBoundingClientRect();camera.x-=(e.clientX-pointer.x)/(r.width/384*camera.zoom);camera.y-=(e.clientY-pointer.y)/(r.height/430*camera.zoom);pointer={x:e.clientX,y:e.clientY};present()});
...
function tick(t){if(!active)return;if(!document.hidden&&t-last>50){const dt=Math.min(.1,(t-last)/1000);state=getState();elapsed+=dt;stepIndividuals(critters,{encounter,state,time:elapsed,dt,reaction:effect&&elapsed<effect.until?effect:null,reduced});drawHabitat(t);last=t}frame=requestAnimationFrame(tick)}
...
function drawHabitat(t){
  ...
  if(effect&&elapsed<effect.until&&['mist','wet-left','wet-all'].includes(effect.id))for(let i=0;i<18;i++){const x=12+(i*29)%(effect.id==='wet-all'?350:82),y=(i*71+t*.05)%420;px(ctx,x,y,2,3,'#91a994')}
...
return {reset,stage,react,zoom,zoomBy:d=>zoom(camera.zoom+d),home:()=>{camera.x=192;camera.y=215;return zoom(1)},start:()=>{if(!active){active=true;frame=requestAnimationFrame(tick)}},stop:()=>{active=false;cancelAnimationFrame(frame);for(const c of critters){setIsopodState(c.el,{posture:c.posture,molt:c.molt,moving:false});c.el.classList.remove('grooming')}},visible:()=>critters.filter(c=>!c.hidden).length};
}
