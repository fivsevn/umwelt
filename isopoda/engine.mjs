...
export function createRun(species='dairy',seed=Date.now()>>>0,habitatId='terrestrial'){
 ...
 const p=speciesById(species),count=config.cohortSize||7,cohort=cohortFor(p.id,seed,count).map(c=>count===1?{...c,stage:'L'}:c);return {habitatId,...(habitatId==='shallow-marine'?{seaweedVersion:1}:{}),...(habitatId==='intertidal'?{intertidalVersion:1}:{}),...(habitatId==='estuary'?{estuaryVersion:1,estuaryShoreVersion:1,shorePoint:1}:{}),startedOn,version:VERSION,narrativeVersion:1,seed:seed>>>0,cohort,day:1,period:0,stage:'choice',humidity:p.wet,temp:23,vent:55,light:54,cover:p.cover,food:1,interventions:0,quiet:0,accuracy:0,maps:0,labels:0,care:0,directTouches:0,directGrabs:0,directMoves:0,groundTaps:0,barkLifts:0,shellCollects:0,collectedShells:[],directRecords:[],interactionDiscoveries:{tap:false,grab:false,lift:false,collect:false},interactionIntent:null,observerContradictions:[],records:[],ending:null,feedback:'',scene:null,...config.defaults,...(habitatId==='estuary'?{flow:42,tide:55}:{})};
}
