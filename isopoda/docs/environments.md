...
淡水腐殖池不再复用三日晨/午/夜节拍。正式运行固定为五次“观察”，围绕同一片叶子的物质变化推进：完整叶片 → 微生物加工 → 啃食与破碎 → 悬浮碎屑 → 再沉积为新的底面。玩家选择改变后续文字的观察框架（食物、遮蔽、位置记录），不决定腐解是否发生。
...
3. 对齐指标、默认值、演进逻辑、动作效果和叙事条件。普通水生正文通常依据 `days * 3` 验证；淡水腐殖池使用 `sequence: freshwater-material` + `turns: 5` 的无日夜五阶段物质循环；深海使用独立 `turns` 与对话路由。
