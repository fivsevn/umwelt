...
function clock(day=state.day,period=state.period){
 ...
 if(config.sequence==='groundwater-pulse'){
  const completed=(state.records||[]).filter(record=>record.kind==='groundwater-pulse').length,index=Number.isInteger(state.scene?.pulseIndex)?state.scene.pulseIndex:Math.min(3,completed);
...
function render(){
 ...
 }else if(config.sequence==='groundwater-pulse'){
  const completed=(Array.isArray(state.records)?state.records:[]).filter(record=>record.kind==='groundwater-pulse').length;
  setWaveText($('#nextBtn'),state.stage==='choice'?t('holdMoment'):completed>=config.turns?t('closeGently'):t('continueObservation'));
...
function drawDrawer(){
 ...
 if(drawerMode==='catalog'){
  ...
  total=Math.max(1,state.records.length);page=Math.max(0,Math.min(page,total-1));$('#drawerTitle').textContent=t('journalTitle');const paper=document.createElement('article');paper.className='journal-paper';if(state.records.length){const r=state.records[page],freshwaterIndex=Math.max(0,Math.floor((state.records.filter((item,i)=>i<=page&&item.kind==='freshwater-material').length-1)/2));let entryDate;if(r.kind==='freshwater-material')entryDate=gameText(`water:freshwater-material:stage:${freshwaterIndex}`,getLanguage())+` · ${(Number.isInteger(r.materialBeat)?r.materialBeat:(state.records.filter((item,i)=>i<=page&&item.kind==='freshwater-material').length-1)%2)+1}/2`;else if(r.kind==='groundwater-pulse'){const pulseIndex=Math.max(0,state.records.filter((item,i)=>i<=page&&item.kind==='groundwater-pulse').length-1);entryDate=gameText(`groundwater:pulse:${Math.min(3,pulseIndex)}:name`,getLanguage())}else entryDate=t('journalEntry',{day:r.day,time:r.time||timeFor(state.seed,r.day,r.period)});paper.append(paragraph(entryDate,'entry-date'),paragraph(r.label,'pencil-mark'),paragraph(r.text))}else paper.append(paragraph(t('journalEmpty')));el.append(paper);
