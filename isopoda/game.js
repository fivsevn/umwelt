...
function restoreEndingLayout(){
 const card=$('#endCard');$('#game').append(card);$('.window-story').classList.remove('sand-ending');
 $('.window-story .record').hidden=false;$('.window-story .bottom-bar').hidden=false;
}
function begin(){restoreEndingLayout();
 ...
 if(state.stage==='ended'){if(state.habitatId==='sandy-surf'){habitat.reset();habitat.home()}showEnd();return}
...
function home(){restoreEndingLayout();playing=false;habitat.stop();$('#playView').hidden=true;$('#endCard').hidden=true;$('#arrivalCard').hidden=true;$('#titleCard').hidden=false;$('#dayLabel').textContent='';$('#startBtn').disabled=false;$('#continueBtn').hidden=!hasRun;$('#continueBtn').textContent=state.stage==='ended'?t('viewEnding'):t('continueObservation');refreshPreview()}
