...
function clock(day=state.day,period=state.period){
 ...
 if(config.sequence==='freshwater-material'){
  const index=Number.isInteger(state.scene?.materialStage)?state.scene.materialStage:Math.min(4,Math.floor((state.records||[]).filter(record=>record.kind==='freshwater-material').length/2));
