...
const endingBodies={
 ...
 estuary:[['最后一处泥边重新被水盖住。你等待过的混合边缘已经移到别处。','The last mud edge is covered again. The mixing boundary you waited beside has moved elsewhere.','最後の泥の縁が再び水に覆われる。待っていた混合の境界は別の場所へ移っている。'],['你改变过入口、植被和沉积。之后记录到的盐度，也包含这些动作留下的条件。','You altered inlets, vegetation, and sediment. Later salinity records also contain the conditions left by those actions.','流入口、植生、堆積を変えた。その後の塩分記録には、その行為が残した条件も含まれている。'],['三天的盐度数字排成一列。河口没有因此变成一条可以固定的线。','Three days of salinity numbers form a column. The estuary does not become a fixed line because of them.','三日分の塩分値が一列に並ぶ。それでも河口は固定できる一本の線にはならない。']],
