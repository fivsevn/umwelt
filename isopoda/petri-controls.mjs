...
export function createPetriControls(viewport,habitat,getState,getLanguage){
 ...
 root.innerHTML='<div class="scope-switch-frame"><button id="scopeToggle" type="button"></button></div><div id="scopePanel" hidden><div class="scope-dials">'+[['Power','power'],['Focus','focus'],['Light','light']].map(([id,key])=>`<div class="scope-knob"><span id="scope${id}Label"></span><button id="scope${id}" class="scope-dial" data-control="${key}" type="button" role="slider" aria-labelledby="scope${id}Label"><span class="dial-face"><i></i></span></button></div>`).join('')+'</div><button id="scopeCenter" type="button">⌖</button></div>';
