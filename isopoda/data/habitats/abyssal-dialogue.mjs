...
export const ABYSSAL_NODES=[
 {
  ...
  options:[
   {id:'contact-wait',label:['再问一次','Ask again','もう一度聞く'],delta:{interpretation:2,maps:1},text:['你把问题重复了一遍。它向前迈了一步：“先告诉我，为什么不动就一定是在等。”','You repeat the question. It takes one step forward. “First tell me why not moving must mean waiting.”','もう一度同じ問いを投げる。它は一歩だけ前へ出る。「まず、動かないことがどうして必ず『待つ』になるのか教えて。」']},
   {id:'contact-name',label:['先叫它的名字','Say its name first','まず名前を呼ぶ'],delta:{attention:2,labels:1},text:['“Bathynomus giganteus。”你念出学名。它停了一会儿：“那是你们给档案用的名字。”','“Bathynomus giganteus.” You say the scientific name. It pauses. “That is the name your archives use.”','「Bathynomus giganteus」と学名を口にする。少し間を置いて、它は言う。「それは君たちの資料庫で使う名前だね。」']},
   {id:'contact-record',label:['先不开口','Stay silent for now','まだ話さない'],delta:{restraint:2,quiet:1},text:['你关掉麦克风，只记下它的位置和朝向。几分钟里，它也没有继续靠近。','You mute the microphone and record only its position and orientation. For several minutes, it does not come any closer.','マイクを切り、位置と向きだけを記録する。数分のあいだ、它もそれ以上近づいてこない。']}
  ]
 },
 {
  id:'time',
  prompt:['几分钟过去，它只移动了不到一个身体长度。你看了一眼计时器，又看了一眼几乎没变的画面。','Several minutes pass. It moves less than one body length. You look at the timer, then back at a scene that has barely changed.','数分が過ぎる。它は一体長にも満たない距離しか進まない。タイマーを見て、ほとんど変わらない画面へ視線を戻す。'],
  options:[
   {id:'time-slow',label:['这里的时间真慢','Time is slow down here','ここは時間が遅い'],delta:{interpretation:2},text:['它停住：“慢，是你把另一只钟带到这里以后才有的词。”','It stops. “Slow is a word that only appears after you bring another clock down here.”','它は止まる。「『遅い』は、別の時計をここへ持ち込んだあとに生まれる言葉だよ。」']},
   ...
   {id:'time-position',label:['只记两个位置','Record two positions only','二つの位置だけ記す'],delta:{restraint:2,attention:1,quiet:1},text:['你在笔记上点下两个坐标，没有画箭头。它看着镜头，什么也没说。','You mark two coordinates in the notebook and draw no arrow between them. It faces the camera and says nothing.','ノートに二つの座標だけを打ち、矢印は引かない。它はカメラを見たまま、何も言わない。']}
  ]
 },
 {
  id:'name',
  prompt:['资料栏一直显示“大王具足虫 / Bathynomus giganteus”。它却已经从画面中央移到一块小石旁边。','The archive still reads “giant isopod / Bathynomus giganteus,” while it has already moved from the center of the frame to a small stone.','資料欄にはずっと「オオグソクムシ / Bathynomus giganteus」と表示されているが、它はすでに画面中央から小石のそばへ移っている。'],
  options:[
   {id:'name-anchor',label:['名字至少告诉我你是谁','The name tells me what you are','名前で何者かは分かる'],delta:{interpretation:1,attention:1,labels:1},text:['“名字能告诉你该翻哪一页。”它说，“但那一页不是我。”','“A name tells you which page to open,” it says. “But that page is not me.”','「名前があれば、どのページを開くかは分かる。」它は言う。「でも、そのページが僕ではない。」']},
 ...
 {
  id:'hunger',
  prompt:['一小片有机残屑从黑水里落下来。它先停住，随后改变方向，慢慢朝那边走。','A small piece of organic debris falls out of the black water. It pauses, changes direction, and slowly heads toward it.','小さな有機物の破片が黒い水から落ちてくる。它は一度止まり、向きを変えて、ゆっくりそちらへ進む。'],
  options:[
   {id:'hunger-motive',label:['你饿了？','Are you hungry?','お腹が空いた？'],delta:{interpretation:2},text:['“你们总喜欢从一条路线直接猜到肚子里。”它没有停下。','“You always like to jump from a route straight into the stomach.” It keeps moving.','「君たちは、一本の進路からすぐ腹の中まで想像したがるね。」它はそのまま進み続ける。']},
 ...
 {
  id:'environment',
  prompt:['仪表显示流速、溶氧和碎屑。它经过的地方却还包括石块的阴影、坡度，以及刚才落下的那片残屑。','The instruments show flow, oxygen, and detritus. The place it moves through also includes stone shadow, slope, and the debris that just fell.','計器には流速、溶存酸素、デトリタスが出ている。けれど它が通る場所には、石の影、傾斜、さっき落ちた破片もある。'],
  options:[
 ...
 {
  id:'translation',
  prompt:['第二天夜里，你终于问：“你真的听得懂我吗？”它停下脚步。黑水里只剩机器低低的嗡声。','On the second night you finally ask, “Do you really understand me?” It stops walking. Only the low hum of the vehicle remains in the black water.','二日目の夜、ついに聞く。「本当に私の言葉が分かるの？」它は歩みを止める。黒い水には機体の低い音だけが残る。'],
  options:[
   ...
   {id:'translation-strip',label:['也许我们只是在互相猜','Maybe we are only guessing each other','互いに推測しているだけかも'],delta:{restraint:2,attention:1},text:['“猜也可以。”它说，“只要别把猜中一次叫成完全理解。”','“Guessing is fine,” it says. “Just do not call one correct guess complete understanding.”','「推測でもいい。」它は言う。「一度当たっただけで、完全に理解したことにしなければ。」']},
   {id:'translation-monologue',label:['不问这个了','Drop the question','この問いはやめる'],delta:{restraint:2,attention:2},text:['你把麦克风放下。它也继续向前。这个问题没有得到结论，却没有妨碍下一步发生。','You lower the microphone. It resumes moving. The question reaches no conclusion, but that does not prevent the next step.','マイクを置く。它もまた歩き始める。問いに結論は出ないが、それでも次の一歩は起きる。']}
  ]
 },
 {
  id:'observer',
  prompt:['载具轻微漂移，探照灯从它背上滑开。它随即转向更暗的一侧。你突然意识到，自己的光也一直在这片环境里。','The vehicle drifts slightly and the lamp slides off its back. It immediately turns toward the darker side. You realize that your light has been part of this environment all along.','機体がわずかに流れ、照明が背中から外れる。它はすぐに暗い方へ向きを変える。自分の光もずっとこの環境の一部だったことに気づく。'],
  options:[
   {id:'observer-reject',label:['你在躲我？','Are you avoiding me?','私を避けている？'],delta:{interpretation:2},text:['“我躲的是光。”它说，“是你把光和自己放进了同一个词。”','“I am avoiding the light,” it says. “You are the one who put the light and yourself into the same word.”','「避けているのは光だよ。」它は言う。「光と自分を同じ言葉に入れたのは君だ。」']},
   {id:'observer-light',label:['只能确定光线变了','All I know is the light changed','確かなのは光が変わったこと'],delta:{restraint:2,attention:1},text:['你记下灯光移动的时间，也记下它转向的时间。两行挨在一起，却没有写成因果。','You record when the light moved and when it turned. The two lines sit beside each other without becoming a causal statement.','光が動いた時刻と、它が向きを変えた時刻を記す。二行は隣り合うが、因果にはしない。']},
 ...
 {
  id:'departure',
  prompt:['第三天最后一个观察时段，它开始朝画面边缘移动。计时器快结束了，它却没有任何要配合你的意思。','In the final observation period of the third day, it starts toward the edge of the frame. Your timer is nearly finished, but it shows no interest in cooperating with it.','三日目、最後の観察時間。它は画面の端へ向かい始める。タイマーは終わりに近いが、它にはそれに合わせる気などない。'],
  options:[
   ...
   {id:'departure-stop',label:['在它离开前停止记录','Stop recording before it leaves','去る前に記録を止める'],delta:{restraint:2,quiet:2},text:['你先关掉记录。屏幕熄灭前，它还在那里。之后发生的事没有进入你的笔记。','You stop the recording first. It is still there when the screen goes dark. Whatever happens next does not enter your notebook.','先に記録を止める。画面が暗くなる瞬間、它はまだそこにいる。その後のことはノートには入らない。']}
  ]
 }
];
