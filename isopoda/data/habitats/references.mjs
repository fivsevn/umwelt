import {READOUT_REFERENCES} from './readout-references.mjs';
// Environment evidence only. Animal-specific evidence belongs to each specimen.
export const HABITAT_REFERENCES={
 'shallow-marine':{
  scope:'近岸浅海 · 藻场 / NEARSHORE SEAWEED BED / 沿岸の藻場',
  entries:[
   {sourceId:'seaweed-flexible-blades',use:'藻体随水流弯曲 / Flexible blades in flow',note:'根部固定、上部形变的视觉依据；游戏中的回弹和拖动不代表力学测量。 / Visual basis for rooted flexible motion, not a mechanical simulation.'},
   {sourceId:'seaweed-balthica-refuge',use:'藻间庇护 / Shelter among blades',note:'波罗的海 I. balthica 实验；支持食物与遮蔽共同影响栖地选择。 / Baltic experiments support food and shelter as habitat factors.'},
   {sourceId:'seaweed-attachment-proxy',use:'抓附的近缘种依据 / Related-species attachment',note:'I. wosnesenskii 的资料仅作属级参考，不能据此给五个可玩物种设置实测阈值。 / Related-species evidence only; no measured thresholds for the five playable taxa.'}
  ],
  limits:'九次观察、09:12–10:02 的场景时刻、水流速度、透光百分比、水深、十八只个体的分布、换叶频率、拨动幅度与回弹速度均为游戏编排。拨藻后的收紧与气泡是扰动的视觉提示，不代表情绪或实测惊逃阈值。物种微栖地来自各标本参考，不宣称同地共存。z 决定实际前后遮挡；layered 藻体的同一程序几何用于绘制、附着和点击。编辑后的稳定对象 ID、位置、镜像、尺度、z 与参数随 JSON 一起保留。 / Nine observations, the 09:12–10:02 clock, current speed, relative light transmission, depth, eighteen individuals, routes, transfer rates and drag motion are authored. Tucking and bubbles mark disturbance, not emotion or measured startle thresholds. Taxa are compared, not asserted to coexist. The shared procedural geometry drives drawing, contact and picking; stable object IDs and parameters survive scene export. / 九回の観察、時刻、個体配置、経路、揺れの数値は創作上の設定。吹き出しは刺激への反応を示し、感情を表さない。'
 },
 'sandy-surf':{scope:'沙滩浪区 / SANDY SURF / 砂浜の波打ち際',entries:[
 {sourceId:'sand-klapow-1972',use:'浪洗区出沙 / Wave-wash emergence',note:'E. chiltoni 的低潮埋藏与满潮出沙支持该物种的沙滩资格；不把采样地点扩展为所有岸段。 / Low-tide burial and high-tide emergence support habitat membership.'},
 ...
 limits:'浪位 0–100 是场景水线上移的相对刻度；浪向由相邻观察的水位差决定，沙面词语与叙事片段对应，不是实测潮位、湿度或现场检测。 / Wash 0–100 is an authored relative shoreline level; direction follows successive scene levels and surface words follow the narrative, not field measurements.  个体位置、几秒内的出入沙循环、抓取、挖坑和局部沙粒抖动是游戏编排，不是实测深度、速度或可靠的野外定位线索。埋藏个体也可以没有提示。E. spinigera 的沙地记录不等于同等充分的行为证据，其动作是属级示意。观察时刻不是当地潮汐预报。 / Positions, accelerated cycles, digging, pickup and grain cues are authored, not measured depths, speeds or reliable field signs. Some animals give no cue. E. spinigera movement is a genus-level illustration, not equally supported species behaviour. Observation times are not tide predictions. / 位置、短い周期、掘削と持ち上げ、砂粒の動きは創作上の表現。静かな砂にも個体がいる。'
 },
 ...
 estuary:{
  ...
  entries:[{sourceId:'estuary-noaa-circulation',use:'潮流、来水与混合 / Tides, river input and mixing',note:'依据河口混合程度与盐度分布会受来水、潮流及地形影响的背景，描绘一段河口岸边。不是所有河口都存在相同的分层或锋面；水流粒子不代表可见的盐度边界。 / Mixing and salinity distribution vary with river input, tides and geometry. Particles are not a visible salinity boundary.'}],
  limits:'近岸左侧显示水深 m、盐度 ‰、流速 cm/s 与定性潮汐状态；右上角为相对观察原点的局部坐标 m，非地理定位。/ Shore readouts show authored water depth, salinity, flow and tide; local coordinates are relative to the observation origin, not geolocation. / 水深・塩分・流速・潮と観察原点からの局所座標を示す。 近岸原型以三个观察点和四种潮位表现岸边变化；水线、雨后出现频率、鱼蟹事件及八次观察均为创作参数，不代表实测尺度、共域物种清单或潮汐时刻。物种名录沿用现有汽水等足类；鱼蟹为环境示意。 / The shore prototype uses authored banks, tides, rain frequency and fauna events; it is not a measured site or co-occurrence inventory. / 岸辺の水位・降雨頻度・生物イベントは創作上の表現で、実測や共存種リストではない。 旧版六次观察、P/Q 水样读数、物件位置和轮廓轨迹均为叙事示意，不是实测数据、盐度耐受阈值或特定物种的行为预测。选择采集的是预先编排的一段观察，不是浏览器逐帧追踪实验。只有选中的证据进入笔记；遮挡或中断后的轮廓不认定为先前个体。各时刻的动画、标记和轨迹以可编辑锚点为参照。保留 estuary-origin-wood、estuary-algae-base、estuary-observation-runnel 三个对象 ID，分别导出六个时刻后再交由开发整合。 / The six observations, sample readings and routes are authored illustrations, not measurements or species-specific responses. Only selected evidence is saved. Keep the three named anchors and export each observation separately. / 六回の観察、値と経路は創作上の模式表現。遮蔽後の輪郭を前の個体とは同定しない。各観察を個別に書き出す。'
 },
 groundwater:{
...
// Append claim-to-source mappings only to the eight revised environments.
for(const [id,scales] of Object.entries(READOUT_REFERENCES)){
 const entry=HABITAT_REFERENCES[id]??={scope:scales.scope,entries:[],limits:''};
 for(const reference of scales.entries){
  const existing=entry.entries.find(item=>item.sourceId===reference.sourceId);
  if(existing){existing.use=reference.use;existing.note+=' '+reference.note}else entry.entries.push(reference);
 }
 entry.limits=(entry.limits?entry.limits+'\n\n':'')+scales.limits;
}
