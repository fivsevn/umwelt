...
export const HABITAT_REFERENCES={
 'shallow-marine':{
  ...
  limits:'九次观察、09:12–10:02 的场景时刻、流势、十八只个体的分布、换叶频率、拨动幅度与回弹速度均为游戏编排。拨藻后的收紧与气泡是扰动的视觉提示，不代表情绪或实测惊逃阈值。物种微栖地来自各标本参考，不宣称同地共存。z 决定实际前后遮挡；layered 藻体的同一程序几何用于绘制、附着和点击。编辑后的稳定对象 ID、位置、镜像、尺度、z 与参数随 JSON 一起保留。 / Nine observations, the 09:12–10:02 clock, flow, eighteen individuals, routes, transfer rates and drag motion are authored. Tucking and bubbles mark disturbance, not emotion or measured startle thresholds. Taxa are compared, not asserted to coexist. The shared procedural geometry drives drawing, contact and picking; stable object IDs and parameters survive scene export. / 九回の観察、時刻、個体配置、経路、揺れの数値は創作上の設定。吹き出しは刺激への反応を示し、感情を表さない。'
 },
