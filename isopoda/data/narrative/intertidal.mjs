...
export const INTERTIDAL_NODES=[
 ...
 ['海退到石沿下面。几只等足类挤在仍湿的藻间，触角伸出去，又收回来。远处的石头看起来很近，中间却少了一段水。','The sea has slipped below the ledge. Several isopods crowd the wet weed, extending and withdrawing their antennae. The next stone looks close; the water between is missing.','海が岩縁の下へ引いた。湿った藻に等脚類が寄り、触角を伸ばしては戻す。隣の石は近いのに、間の水が途切れている。'],
 [['看湿藻里的触角','Watch the antennae','藻の中の触角を見る'],['沿池边看一圈','Look around the pool','池の縁をたどる'],['记下这道低水线','Mark the low waterline','低い水際を記す']],
 [['触角一前一后扫过藻面，身体还留在阴影里。小小的活动没有停，只是挤进了更窄的地方。','Antennae sweep the weed in turn. The bodies stay in shade. Movement continues in a smaller space.','触角が交互に藻を探る。身体は陰に残る。狭い場所でも動きは続く。'],['池底还留着水，石脊却露出来了。此刻能走的地方，被几段干湿相接的边缘分开。','Water remains in the pool, but the ridges are exposed. Wet and dry edges divide the available routes.','池底には水が残り、岩の稜は露出している。乾湿の境が道を分ける。'],['你在纸上留下一道短线。它只是这一次退潮的位置。','You draw a short line: the position of this low tide.','紙に短い線を引く。今回の干潮の位置だけを示す線だ。']]),
 ...
 [['跟看一只游过的身体','Follow a crossing body','泳ぐ個体を追う'],['留在原来的湿藻边','Stay by the first weed','最初の藻のそばに留まる'],['记下最宽的水面','Mark the widest water','広がった水を記す']],
 ...
 ['几段水路断开了。身体渐渐靠回低处的藻和岩缝，藤壶把蔓足收进壳里。水面小了，近处的细小动作反而容易看清。','Several water routes break apart. Bodies draw back toward low weed and cracks; barnacles fold their cirri inside. With less water, nearby movements become easier to see.','いくつかの水路が切れる。個体は低い藻や割れ目へ寄り、フジツボは蔓脚をしまう。水が減り、近くの小さな動きが見やすい。'],
 [['看藻下的小动作','Watch beneath the weed','藻の下の動きを見る'],['看合拢的壳口','Watch the closing shells','閉じる殻口を見る'],['记下断开的水路','Mark the broken routes','切れた水路を記す']],
 [['触角从藻下伸出，步足换了一个抓点。活动缩在湿处，没有跟着海一起离开。','Antennae reach from under weed; legs change their grip. Activity stays in the wet refuge.','藻の下から触角が出て、脚がつかむ場所を変える。湿った避難場所で動きが続く。'],['刚才扫水的细线不见了，壳还留在同一块石头上。','The fans that swept the water disappear; the shells remain on the same stone.','水を払った扇は消え、殻は同じ石に残る。'],['你把连线停在露出的石脊前。水下的两边，现在需要分别看。','You stop the line at the exposed ridge. The two wet sides must now be watched separately.','露出した稜で線を止める。濡れた両側を別々に見る。']]),
 row(['又一次低水','Low water again','再び低い水'],
 ['水回到接近起点的高度。湿藻里仍有触角在动，你却不能凭一个相似的位置认出先前的那一只。海走完一轮，池里没有回到原样。','Water returns near its starting height. Antennae still move in the weed, but a familiar position cannot identify the earlier animal. A tide has passed; the pool is not an exact repeat.','水は初めに近い高さへ戻る。藻で触角は動くが、似た位置だけでは先ほどの個体と分からない。一巡しても池は同じには戻らない。'],
 [['停在湿藻旁','Stay by the wet weed','湿った藻のそばに留まる'],['重看这一轮的水路','Review the changing routes','変わった道を見返す'],['合上这页水线','Close the waterline page','水際の頁を閉じる']],
...
export function intertidalText(key,lang='zh'){
 const p=key.split(':');let r=COPY[p[1]];
...
export function intertidalScene(s){const i=intertidalIndex(s),key=`intertidal:node:${i}`;return {id:key,kind:'intertidal-tide',title:key+':name',text:key+':prompt',activity:key+':name',storyKey:key,options:INTERTIDAL_NODES[i].options.map((o,j)=>({id:`tide-${i}-${j}`,label:key+`:option:${j}:label`,text:key+`:option:${j}:text`,delta:{},animation:['tide-body','tide-water','tide-record'][j]}))}}

const catalogRow=(key,values)=>({key,locales:{zh:values[0],en:values[1],ja:values[2]},refs:{habitat:['intertidal']}});
export const TEXT_CATALOG=[...Object.entries(COPY).map(([k,v])=>catalogRow('intertidal:'+k,v)),...INTERTIDAL_NODES.flatMap((n,i)=>[...['name','prompt'].map(k=>catalogRow(`intertidal:node:${i}:${k}`,n[k])),...n.options.flatMap((o,j)=>['label','text'].map(k=>catalogRow(`intertidal:node:${i}:option:${j}:${k}`,o[k])))])];
