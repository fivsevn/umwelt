...
export const AQUATIC_SPECIES=AQUATIC_TAXA.map(([id,taxon,authority,habitat,suborder,family,url,microhabitat,maxLength,color])=>({
 ...
 evidenceIds:['aquatic-'+id,...(id==='balthica'?['seaweed-balthica-refuge']:[]),...(id==='granulosa'?['intertidal-bmig-granulosa','intertidal-naturalis-granulosa']:[])],evidence:{status:'literature_supported',claims:[{claim:microhabitat,url},...(id==='granulosa'?[{claim:'Occurs under intertidal stones and amongst seaweed.',url:'https://bmig.org.uk/species/idotea-granulosa'},{claim:'May occur free swimming between tidemarks; animation trajectories and speeds are authored.',url:'https://ns-zooplankton.linnaeus.naturalis.nl/linnaeus_ng/app/views/species/taxon.php?id=132141'}]:[])]},
 ...
 profile:{scientificNotes:id==='balthica'?['藻间食物与庇护的实验见当前标本参考。游戏中的换叶路线和速度是创作参数，不是实测轨迹。 / Food and refuge experiments are linked below; blade-transfer routes and speeds are authored. / 藻間の餌と隠れ場の実験を参照。経路と速度は創作上の設定。']:INTERTIDAL_SCIENCE[id]||null,adultLengthMm:null,adultLengthRangeMm:null,reportedMaximumLengthMm:maxLength,ecology:[habitat],microhabitat:[microhabitat],behaviour:['aquatic locomotion'],notableMorphology:[family+' dorsal body plan'],diagnosticNotes:['Approximate family silhouette; microscopic and sexual diagnostics omitted.'],conglobation:family==='Sphaeromatidae'?'full':'none',evidenceIds:['aquatic-'+id]},
