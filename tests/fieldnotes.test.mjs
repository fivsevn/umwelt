...
import {MOTIONS,makeIndividuals,stageIndividuals,stepIndividuals} from '../isopoda/behaviors.mjs';
...
test('all encounter motions animate five bounded stable individuals, with correct molt halves',()=>{
 ...
 for(const e of ENCOUNTERS){const group=makeIndividuals(42);stageIndividuals(group,e);const ids=group.map(c=>c.seed);let moved=false;for(let i=0;i<240;i++){const before=group.map(c=>[c.x,c.y]);stepIndividuals(group,{encounter:e,state:{humidity:65},time:i*.1,dt:.1});moved ||= group.some((c,j)=>c.x!==before[j][0]||c.y!==before[j][1]);assert.equal(group.length,5);for(const c of group)assert.ok(Number.isFinite(c.a)&&c.x>=24&&c.x<=355&&c.y>=30&&c.y<=400);if(e.motion==='molt')assert.equal(group[0].molt,e.molt)}assert.ok(moved);assert.deepEqual(group.map(c=>c.seed),ids)}
});
test('reduced motion freezes position; disturbing and waiting alter defensive encounter',()=>{
 const e=ENCOUNTERS.find(e=>e.motion==='defend'),g=makeIndividuals(2);stageIndividuals(g,e);const p=g.map(c=>[c.x,c.y]);stepIndividuals(g,{encounter:e,state:{},time:3,reduced:true});assert.deepEqual(g.map(c=>[c.x,c.y]),p);assert.ok(g.every(c=>!c.moving));
 stepIndividuals(g,{encounter:e,state:{},time:3,reaction:{mode:'quiet'}});assert.equal(g[0].posture,'normal');stepIndividuals(g,{encounter:e,state:{},time:3,reaction:{mode:'disturb'}});assert.equal(g[0].posture,'curled');
});
