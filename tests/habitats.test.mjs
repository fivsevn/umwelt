...
test('all configured habitats complete their full duration with eligible animals and localized distinct scenes',()=>{
 for(const h of HABITATS){
  ...
  const scenes=new Set();let turns=0;const expectedTurns=habitatConfig(s).turns||h.days*3;
  while(s.stage!=='ended'){
   const scene=ensureScene(s);scenes.add(scene.text);if(s.habitatId==='petri-dish')s.microscope={gateVersion:2,completed:{[(s.day-1)*3+s.period]:true}};
...
test('freshwater material layouts use the five authored filename-mapped scenes',()=>{
 ...
 assert.deepEqual(FRESHWATER_STAGE_LAYOUTS.map(layout=>layout.objects.length),[24,29,38,42,46]);
 ...
 assert.equal(FRESHWATER_STAGE_LAYOUTS[4].objects.filter(object=>object.type==='freshwater-detritus-01').length,7);
