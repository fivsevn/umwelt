...
test('new sprites retain conservative scaffolds while evidence changes readable morphology',()=>{
 ...
 const giant=speciesById('giganteus');assert.equal(giant.taxonomy.acceptedScientificName,'Bathynomus giganteus');assert.equal(giant.game.habitats[0],'abyssal');assert.equal(giant.game.habitatEligible,false);assert.equal(giant.visual.morphologyKey,'cirolanidAbyssal');assert.equal(giant.names.zhCN,'大王具足虫');assert.equal(giant.names.ja,'ダイオウグソクムシ');
