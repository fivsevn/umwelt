...
(async()=>{
 ...
 for(const width of [390,900])for(const lang of ['zh','en','ja','isopod']){
  ...
  for(const habitat of ['freshwater','intertidal','shallow-marine']){
   ...
   for(let turn=0;turn<9;turn++){
    const text=await p.locator('#observation').innerText();assert.ok(text&&!text.includes('water:'));
  ...
  assert.equal(await p.locator('#endCard').isVisible(),true);assert.equal(await p.evaluate(()=>JSON.parse(localStorage.getItem('isopoda-fugue-v4')).habitatId),'shallow-marine');
  ...
  await p.close();console.log('PASS',width,lang,'three aquatic runs, continue and reload');
 }
 ...
 for(const lang of ['en','ja','isopod','zh'])await p.evaluate(async lang=>{const m=await import('./i18n.mjs?v=aquatic-1');m.setLanguage(lang)},lang);
 await p.close();
