...
(async()=>{
 ...
 for(const preset of presets){
  ...
  const expectedFilename=preset==='freshwater'?'habitat-layout-freshwater-pool-stage-01-leaf.json':preset==='estuary'?'habitat-layout-brackish-estuary-observation-01-mark.json':`habitat-layout-${names[preset]}.json`;
  assert.equal(exported.suggestedFilename(),expectedFilename,sceneDownload);
 ...
 for(const [width,height] of [[320,568],[390,844],[768,900],[1120,800],[1121,800],[1440,900]]){
  ...
  await page.locator('.scene-transfer summary').click();assert.equal(await page.locator('#sceneText').isVisible(),false);await page.locator('.scene-transfer summary').click();
