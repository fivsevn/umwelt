...
const {click:pointerClick}=require('../support/browser-pointer.cjs');
...
(async()=>{
 ...
 const click=selector=>pointerClick(page,selector);
 ...
 await page.locator('#scene').evaluate(e=>e.scrollIntoView({block:'center',behavior:'instant'}));let p=await point(180,200);await page.mouse.click(p.x,p.y);
 ...
 for(const [width,height] of [[320,568],[390,844],[768,900],[1120,800],[1121,800],[1440,900]]){
  ...
  await page.locator('#scene').evaluate(e=>e.scrollIntoView({block:'center',behavior:'instant'}));
  const before=await snapshot();await page.locator('#scene').evaluate(e=>e.scrollIntoView({block:'center',behavior:'instant'}));
