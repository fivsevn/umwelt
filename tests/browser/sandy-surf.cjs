...
try{for(const width of [320,390,1440]){
 ...
 await down(p);await page.waitForTimeout(650);assert.equal((await snap(id)).mode,'grabbed');assert.equal((await snap(id)).hidden,false);assert.ok(await page.evaluate(id=>window.__sandTest.bubbles().some(b=>b.id===id&&['!','?'].includes(b.cue)),id));await page.screenshot({path:`${out}/${engine}-${width}-dig.png`,fullPage:true});
 ...
 for(const [name,wx,wy] of [['sand',270,115],['water',270,365]]){
 const q=await page.evaluate(({wx,wy})=>{const r=document.querySelector('#habitat').getBoundingClientRect(),v=window.__sandTest.view();return {x:r.left+(wx-v.sx)*v.scale,y:r.top+(wy-v.sy)*v.scale}},{wx,wy});
 await down(q);await up();await page.waitForTimeout(180);await page.screenshot({path:`${out}/${engine}-${width}-${name}.png`,fullPage:true});
 }
