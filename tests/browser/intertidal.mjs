 ...
 try {for(const [width,height] of [[320,568],[390,844],[1440,900]]){
  ...
  for(let i=0;i<9;i++){
   ...
   for(const lang of ['zh','en','ja','isopod']){
    await page.evaluate(async lang=>(await import('./i18n.mjs')).setLanguage(lang),lang);
   ...
   await page.evaluate(async()=>(await import('./i18n.mjs')).setLanguage('zh'));
   if(i===0||i===4){
  ...
  await page.goto(base+'/isopoda/habitat.html');await page.waitForFunction(()=>document.querySelector('[data-preset="intertidal"]').onclick);await click(page,'[data-preset="intertidal"]');assert.equal(await page.locator('.habitat-reference').count(),3);
